      ...
      <Link to={fullRoute.aGlobalRoute.aUnprotectedRoute.aHomeRoute} >
        <div className="flex items-center space-x-2">
          <Avatar>
            ...
            <AvatarFallback>{getInitials(user ? `${user.eFirstname}` : "Happify", user ? `${user.eLastname}` : "Happify"  )}</AvatarFallback>
          </Avatar>
          <div className='hidden sm:block' >
            <p className="text-sm font-medium leading-none">{user ? `${user.eFirstname} ${user.eLastname}` : `Happify`}</p>
            {/* <p className="text-sm text-muted-foreground">{user ? `${user.eEmail}` : `someone@happify.com`}</p> */}
          </div>
        </div>
      </Link>
    </React.Fragment>
